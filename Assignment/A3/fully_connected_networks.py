...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        N = x.shape[0]
        reshape_x = x.view(N, -1)
        out = torch.mm(reshape_x, w) + b
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
        cache = (x, w, b)
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
        dx = torch.mm(dout, w.t()).reshape_as(x)
        reshape_x = x.view(x.shape[0], -1)
        dw = torch.mm(reshape_x.t(), dout)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for j in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dh = Dropout.backward(dh, dropout_caches[j])
            dh, grads[f'W{j}'], grads[f'b{j}'] = Linear_ReLU.backward(dh, relu_caches[j])
...
def rmsprop(w, dw, config=None):
    ...
    next_w = w - config.get('learning_rate') / (torch.sqrt(cache) + config.get('epsilon')) * dw
    config['cache'] = cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
